...
window.NexraAuth = {
    ...
    adjustWrapperHeight(face) {
        // Adjust the container height based on the active face so it doesn't clip
        const faces = {
            'login': 'ag-login-face',
            'register': 'ag-register-face',
            'forgot': 'ag-forgot-face'
        };
        setTimeout(() => {
            const activeFace = document.getElementById(faces[face]);
            const wrapper = document.querySelector('.ag-3d-wrapper');
            if(activeFace && wrapper) {
                wrapper.style.height = `${activeFace.offsetHeight}px`;
            }
        }, 100);
    },
    ...
    async registerEmail(e) {
        ...
        if(pwd.length < 6) {
            return NexraApp.showToast('Password must be at least 6 characters', 'error');
        }
    ...
    async resetPassword(e) {
        ...
        try {
            ...
            NexraApp.showToast('Password reset link sent to your email.', 'success');
